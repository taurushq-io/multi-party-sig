...
func (round *output) ProcessMessage(msg *pb.Message) error {
	...
	if len(body.SchF) != round.S.Threshold+1 {
		return fmt.Errorf("refresh_old.output.ProcessMessage(): party %s: wrong number of Schnorr commitments", j)
	}
	for l := range body.SchF {
		// if in refresh_old, we know the constant coefficient is 0 so no need to check
		if !round.keygen && l == 0 {
			continue
		}
		schX, err := body.SchF[l].Unmarshal()
		if err != nil {
			return fmt.Errorf("refresh_old.output.ProcessMessage(): party %s: unmarshal schX: %w", j, err)
		}
		X := partyJ.polyExp.Coefficients()[l+1]
		if !zksch.Verify(round.H.CloneWithID(j), partyJ.A[l], X, schX) {
			return fmt.Errorf("refresh_old.output.ProcessMessage(): party %s: failed to validate sch proof for coef %d", j, l)
		}
	}
	...
	if xJdec.Cmp(xJ.BigInt()) != 0 {
		return fmt.Errorf("refresh_old.output.ProcessMessage(): party %s: decrypted share is not in correct range", j)
	}
	...
	if !X.Equal(vss) {
		return fmt.Errorf("refresh_old.output.ProcessMessage(): party %s: failed to validate share from VSS", j)
	}
	...
	if !modPublic.Verify(round.H.CloneWithID(j), body.GetMod()) {
		return fmt.Errorf("refresh_old.output.ProcessMessage(): party %s: mod proof failed to verify", j)
	}

	// verify zkprm
	prmPublic := zkprm.Public{Pedersen: partyJ.Pedersen}
	if !prmPublic.Verify(round.H.CloneWithID(j), body.GetPrm()) {
		return fmt.Errorf("refresh_old.output.ProcessMessage(): party %s: prm proof failed to verify", j)
	}
...
func (round *output) GenerateMessages() ([]*pb.Message, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("refresh_old.output.GenerateMessages(): sum polynomial exponent: %w", err)
	}
	...
	if err = round.S.RecomputeSSID(); err != nil {
		return nil, fmt.Errorf("refresh_old.output.GenerateMessages(): compute SSID: %w", err)
	}

	if err = round.S.Validate(); err != nil {
		return nil, fmt.Errorf("refresh_old.output.GenerateMessages(): validate new session: %w", err)
	}

	if !round.S.KeygenDone() {
		return nil, errors.New("refresh_old.output.GenerateMessages(): session is not in post keygen state")
	}
