...
func SignTaprootAdaptor(config *TaprootConfig, signers []party.ID, adaptorPoint curve.Secp256k1Point, messageHash []byte) protocol.StartFunc {
	publicKey, err := curve.Secp256k1{}.LiftX(config.PublicKey)
	if err != nil {
		return func([]byte) (round.Session, error) {
			return nil, err
	...
	return sign.StartSignAdaptor(normalResult, signers, adaptorPoint, messageHash)
}
