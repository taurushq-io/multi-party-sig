...
func (r *round2) GetMessagesOut() ([]*Message, error) {
	...
	for i, otherPartyID := range r.otherIDs {
		...
		betaNeg := arith.MustSample(zk.TwoPowLPrime)
		beta := r.group.Scalar().SetBytes(betaNeg.Bytes())
		D, s := verifier.Paillier.Affine(verifier.K, arith.GetBigInt(r.gamma), arith.GetBigInt(beta))
		F, rho := selfParty.Paillier.Enc(arith.GetBigInt(beta))
		proofGamma := zk.NewAffineGroupCommitmentRange(r.group, selfParty.Paillier, verifier.Paillier, verifier.Pedersen, verifier.K, D, F, selfParty.Gamma, r.gamma, beta, s, rho)
		verifier.beta = beta.Neg(beta)

		betaNegHat := arith.MustSample(zk.TwoPowLPrime)
		betaHat := r.group.Scalar().SetBytes(betaNegHat.Bytes())
		DHat, sHat := verifier.Paillier.Affine(verifier.K, arith.GetBigInt(r.secret.ECDSA), arith.GetBigInt(betaHat))
		FHat, rhoHat := selfParty.Paillier.Enc(arith.GetBigInt(betaHat))
		proofX := zk.NewAffineGroupCommitmentRange(r.group, selfParty.Paillier, verifier.Paillier, verifier.Pedersen, verifier.K, DHat, FHat, selfParty.ECDSA, r.secret.ECDSA, betaHat, sHat, rhoHat)
		verifier.betaHat = betaHat.Neg(betaHat)

		proofLog := zk.NewLog(r.group, selfParty.Paillier, verifier.Paillier, verifier.Pedersen, selfParty.G, selfParty.Gamma, r.group.Point().Base(), r.gamma, r.gammaEncNonce)
